////////////////////////////////////////////////////////////////////
//jsfl 运行路径
var jsflUrl=fl.configURI+'WindowSWF/OpenFire/SpeTools/jsfl';
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
CopyMotionErrorStrings.FOLDER_LAYER = "此图层是一个文件夹。要复制动作，在一个具有内容的图层上选择帧。";
CopyMotionErrorStrings.GUIDE_LAYER = "要连同运动引导层一起复制动作，选择引导层上的\n动画，而不要选择运动引导层。\n运动引导层会随动画自动复制。";
CopyMotionErrorStrings.MULTIPLE_SPANS = "选择了多个帧范围。\n要复制动作，在一个图层上选择一个连续的帧范围。";
CopyMotionErrorStrings.NO_CONTENT = "要复制动作，所选择的第一个帧应该有内容。";
CopyMotionErrorStrings.SAVE_ERROR = "在磁盘上存储动作时出错。";
CopyMotionErrorStrings.NO_OBJECTS = "在帧 {1} 上没有对象。";
CopyMotionErrorStrings.MULTIPLE_OBJECTS = "在帧 {1} 上有一个以上的对象。";
CopyMotionErrorStrings.NO_CONTAINER = "帧 {1} 上的形状没有容器。\n要复制动作，将其转换为元件或组。";


PasteMotionErrorStrings = {};
PasteMotionErrorStrings.NO_SELECTION = "尚未进行选择。要粘贴动作，在舞台上选择一个对象。";
PasteMotionErrorStrings.MULTIPLE_SPANS = "选择了多个帧范围。\n要粘贴动作，在一个图层上选择一个连续的帧范围。";
PasteMotionErrorStrings.NO_OBJECTS = "在帧 {1} 上没有对象。";
PasteMotionErrorStrings.MULTIPLE_OBJECTS = "在帧 {1} 上有一个以上的对象。";
...
saveMotionXML = function(contents, motionName)
{
	...
	if (!motionName)
	{
		fileURL = fl.browseForFileURL("save", "将动画 XML 另存为...");
		if (!fileURL || !fileURL.length)
			return false;
	}
	else
	{
		var motionsFolder = jsflUrl+'/';
...
loadMotionXML = function(motionName)
{
	...
	if (!motionName)
	{
		...
		var macFormatStr = "XML 文件|TEXT[*.xml||";
		var winFormatStr = "XML 文件|*.xml||";
		fileURL = fl.browseForFileURL("open", "打开动画 XML", obsoleteDWPreviewAreaObject, macFormatStr, winFormatStr);
		if (!fileURL || !fileURL.length)
			return '';
...
importMotionXML = function(options)
{
	...
	var options = fl.getDocumentDOM().xmlPanel(jsflUrl+'/PasteMotionSpecialDialog.xml');
	importer.importXMLString(xmlString, options);
	savePasteOptions(importer.options);
	return true;
}
...
savePasteOptions = function(options)
{
	...
	var templateURL = jsflUrl + "/PasteMotionSpecialDialogTemplate.xml";
	...
	var dialogURL = jsflUrl + "/PasteMotionSpecialDialog.xml";
	if (!FLfile.write(dialogURL, contents))
	{
		fl.trace("尝试将“粘贴动画”设置保存到磁盘时出错。");
		return false;
	}
...
copyMotionAsAS3 = function(instanceName)
{
	...
	if (!id || id == '')
	{
		...
		do
		{
			...
			var id = prompt("要在 ActionScript 中使用的实例名称: ",
				detectedName);
	...
	var templateURL =jsflUrl + '/CopyMotionAsAS3Template.as';

	if (!FLfile.exists(templateURL))
	{
		fl.trace("错误: 缺少模板文件 " + templateURL);
		return false;
	}
...
function copyMotion_spe(){
	//fl.trace(jsflUrl)
	var contents= copyMotionAsXML()
	var moveFileXml=jsflUrl+'/coyeAndPasteMovieTemp.xml';
	if (!FLfile.write(moveFileXml, contents))
	{
		fl.trace("尝试将“粘贴动画”设置保存到磁盘时出错。");
		return false;
	}
...
function pasteMotion_spe(){
	var moveFileXml=jsflUrl+'/coyeAndPasteMovieTemp.xml';
	var ending = moveFileXml.slice(-4);
	if (FLfile.exists(moveFileXml) && ending == '.xml')
	{
		var contents = FLfile.read(moveFileXml);
		//fl.trace('读取动画数据成功')
	}else{
		fl.trace('读取动画数据不成功')
		return;
	...
	importer.importXMLString(contents, options);
	savePasteOptions(importer.options);
}
