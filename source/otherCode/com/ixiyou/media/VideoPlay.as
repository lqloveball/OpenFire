package com.ixiyou.media 
{
	...
	public class VideoPlay extends MovieClip 
	{
		...
		public function VideoPlay(_skinTemp:MovieClip,confing:Object=null) 
		{
			if (confing) {
				if(confing.playSliderMode!=null)playSliderMode=confing.playSliderMode
			}
			skin = _skinTemp
			videoloader = new VideoLoader('', { bufferTime:1, bufferMode:true,
												autoAdjustBuffer:true,
						autoPlay:true,scaleMode:'stretch',width:videoRect.width,height:videoRect.height})
		...
		public function set skin(value:MovieClip):void 
		{
			...
			if (skin.btn_outFullScreen) {
				btn_outFullScreen = skin.btn_outFullScreen as MovieClip;
				//ButtonUtil.setButton(btn_fullScreen)
				btn_outFullScreen.buttonMode = true
				btn_outFullScreen.mouseChildrenf=false
				btn_outFullScreen.addEventListener(MouseEvent.CLICK,btn_outFullScreenFun)
			}else {
				trace('no skin.btn_outFullScreen')
			}
			if (skin.fullBg) {
				fullBg = skin.fullBg
				skin.removeChild(fullBg)
			}else {
				trace('no skin.fullBg')
			}
			
			if (skin.playSlider) {
				playSlider = skin.playSlider as MovieClip;
				playSlider_x=playSlider.x
				playSlider_y = playSlider.y
				...
				if(playSlider.slider_bg)slider_bg = playSlider.slider_bg as MovieClip;
				
				sliderBg = playSlider.sliderBg;
				sliderBg.buttonMode = true;
				sliderBg.addEventListener(MouseEvent.CLICK, sliderBgCLick)
				//trace(slider.getRect(slider))
				if (slider.getRect(slider).x<0) {
					_playSliderMode='type1'
				}else {
					_playSliderMode='type2'
				}
		...
		private function sliderDownFun(e:MouseEvent):void {
			...
			rect = new Rectangle(loading.x, slider.y, w, 0)
			sliderDown=true
		...
		private var playSlider_x:Number
		private var playSlider_y:Number
		
		private function btn_outFullScreenFun(e:MouseEvent):void 
		{
		...
		{
			DebugOutput.add('btn_fullScreenFun 1:>>',stage.displayState)
			...
			DebugOutput.add('btn_fullScreenFun',stage.displayState)
			fullScreen()
		}
		/**
		 * 全屏
		...
		{
			if (!stage) return
			DebugOutput.add('stage.displayState:',stage.displayState)
			...
			if (stage.displayState == StageDisplayState.FULL_SCREEN) {
				if (fullBg) {
					...
					fullBg.x = fullBg.y = 0;
					fullBg.width=this.stage.stageWidth
					fullBg.height=this.stage.stageHeight
				}
				if (videoloader&&videoloader.content) {
					stage.addChild(videoloader.content);
					Sprite(videoloader.content).width = this.stage.stageWidth
					Sprite(videoloader.content).height = this.stage.stageHeight
				}
				
				stage.addChild(this);
				this.x=((this.stage.stageWidth-videoBg.width)/2)>>0
				this.y = (this.stage.stageHeight - this.height)
				
				btn_fullScreen.visible=false
				videoBg.visible = false;
				btn_outFullScreen.visible = true
				btn_outFullScreen.x = this.stage.stageWidth
				btn_outFullScreen.y = 0
				stage.addChild(btn_outFullScreen)
				...
				oldParent.addChildAt(this,0)
				this.x=oldx
				this.y = oldy
				btn_fullScreen.visible = true;
				videoBg.visible = true;
				btn_outFullScreen.visible = false;
				//playSlider.x=playSlider_x
				//playSlider.y=playSlider_y
				
				if (fullBg) {
					if(stage.contains(fullBg))stage.removeChild(fullBg)
					//fullBg.x = fullBg.y = 0;
					//fullBg.width=this.stage.stageWidth
					//fullBg.height=this.stage.stageHeight
				}
		...
		private var btn_outFullScreen:MovieClip;
		private var fullBg:MovieClip;
		...
		public function upBuffer():void {
			if (mc_preloader) {
				//if (videoloader && videoloader.bufferMode) 
				mc_preloader.visible = isBuffer
		...
		public function upPlayState():void {
			if (skin) {
				//trace(btn_play,btn_pause,btn_screen_play,btn_screen_pause)
				if (videoloader) {
					if (videoloader.videoPaused) {
