package com.ixiyou.speUI.mcontrols
{
    ...
    public class ActionScriptEidtPanel extends MSprite {
		private var eidtPanel:AdvancedTextEditor;
        public function ActionScriptEidtPanel() {
			setSize(400,300)
            eidtPanel = new AdvancedTextEditor() ;
            addChild(eidtPanel)
            upSize();
        }
        override public function upSize():void {
			if(!eidtPanel)return
			 eidtPanel.width = width;
			eidtPanel.height = height;
		}
...
class TextEditUI extends UIControl {
	...
	private var _eidtBool:Boolean=true
    /**
	 ...
	 * 是否可编辑
	 */
	public function get eidtBool():Boolean 
	{
		return _eidtBool;
	}
	
	public function set eidtBool(value:Boolean):void 
	{
		_eidtBool = value;
		if (eidtBool) textField.type = TextFieldType.INPUT;
		else textField.type = TextFieldType.DYNAMIC;
	}
...
class TextEditor extends TextEditorBase {
    ...
    private function createDebugMenu():ContextMenu {
        ...
        createMenuItem("打开文件", open);
        createMenuItem("保存文件", save);
        createMenuItem("撤销", undo, function ():Boolean { return historyManager.canBack; }, true);
        createMenuItem("重做", redo, function ():Boolean { return historyManager.canForward; } );
		/*
    ...
    private function keyDownHandler(event:KeyboardEvent):void {
       ...
       trace(event.keyCode )
        ...
        if ((event.keyCode == Keyboard.SPACE && event.ctrlKey) || (event.keyCode == 191 && event.ctrlKey)) {
			//trace('进行代码提示')
            activateCodeHint();
            event.preventDefault();
            preventFollowingTextInput = true;
