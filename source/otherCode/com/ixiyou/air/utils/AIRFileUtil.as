package com.ixiyou.air.utils 
{
	...
	public class AIRFileUtil
	{
		 ...
		 * @param	value  
		 * @return 应用程序的安装目录
		 * applicationDirectory('images') // app:/images
		 */
		 ...
		 * @return
		 */
		public static function saveFileStreamAsync(file:File,bytes:ByteArray,append:Boolean=false):void {
			var myFileStream:FileStream = new FileStream(); 
			var bytes:ByteArray = new ByteArray(); 
			var completeHandler:Function=function(e:Event):void  
			{ 
			...
			myFileStream.addEventListener(Event.COMPLETE, completeHandler); 
			try {
				if (!append) myFileStream.openAsync(file, FileMode.WRITE);
				else myFileStream.openAsync(file, FileMode.APPEND);
