package com.ixiyou.magazine 
{
	...
	public class MagaPageFlip extends Sprite
	{
		...
		private function turnPage(pt:Point, bool:Boolean = true, mousePt:Point = null):void {
			...
			//trace(pt,bool,mousePt)
			TweenMax.to(mousePt,.3,{ time:.3, x:_w, y:_h,
				onUpdate:function(book:MagaPageFlip,pt:Point):void {
					obj = PageFlip.computeFlip(new Point(book.x, book.y), pt, book.width / 2, book.height, true, 1);
					book.drawBitmapBook(obj)
				},
				onUpdateParams:[this,pt],
				onComplete:function(book:MagaPageFlip,bool:Boolean):void {
					book.renderBox.visible = false
					if (bool) {
						book._pageIndex = book.nextIndex;
					}
					book.setPageIndex(book.pageIndex)
					book.turnBool = false
					book._bookMask.x = 0
					book._bookMask.width=width
					dispatchEvent(new Event('upPage'))
				},
				onCompleteParams:[this,bool]
			})
		}
		/**
		 * 设置第下显示页面
