package com.ixiyou.utils.display
{
	...
	*/
	public class BitmapDataUtils 
	{
		 ...
		 * @param area 只获取的对象大小
		 * @param backgroundColor 背景用来填补的颜色 -1 代表透明通道
		 * @return 
		*/
		static public function getBitmapDataBySize(d:DisplayObject, size:Rectangle, area:Rectangle = null, backgroundColor:int = -1):BitmapData
		{
			...
			old_bmd = new BitmapData(d.width*s,d.height*s,true,0x00FFFFFF);
			old_bmd.draw(d,max,null,null,null,true);
			
			var new_bmd:BitmapData;
			if(backgroundColor == -1){
				new_bmd = new BitmapData(size.width,size.height,true,0x00FFFFFF);
			}else{
